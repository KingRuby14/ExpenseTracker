...
router.get("/dashboard", auth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      balance:
        (incomeTotalAggregate ? incomeTotalAggregate.total : 0) -
        (expenseTotalAggregate ? expenseTotalAggregate.total : 0),
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
...
router.get("/download/incomes", auth, async (req, res) => {
  try {
    ...
    const rows = await Income.find(filter).sort({ date: -1 }).lean();
